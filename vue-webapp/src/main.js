...
// 把组件注册成全局组件
Vue.use(Button)   // Vue.use() 调用Button的install()方法。
Vue.use(Tabbar)
Vue.use(TabbarItem)
Vue.use(NoticeBar)
Vue.use(Search)
...
Vue.directive('chart', function(el, binding) {
  console.log('自定义指令')
  console.log('指令作用的节点', el)
  console.log('指令binding', binding)
  const { upper } = binding.modifiers
  el.innerText = upper ? binding.value.toUpperCase() : binding.value
})
